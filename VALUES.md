...
There is no telemetry merely because telemetry is customary. If a future
feature cannot work without collecting sensitive information, its need, scope,
retention, recipients, and controls must be explicit before implementation.

An infrastructure breach should reveal no meaningful user activity. Public
source code is expected; private user behavior is not part of the product.
...
Security, performance, efficiency, accessibility, documentation, and a superb
terminal and native-app experience are one quality system. Helpful errors,
careful writing, fast feedback, tasteful visual design, reliable removal, and
clean architecture are not decoration around the software. They are evidence
that contributors respected the user's time and agency.

We choose durable solutions over shortcuts. We correct root causes instead of
weakening tests or gates. We design for growth without speculative machinery,
and we add complexity only when it earns its permanent cost.
...
We support open projects and organizations that share those broad virtues even
when their technical tastes differ from ours. We ignore ordinary disagreement.
We respect authorship, copyright, trademarks, licenses, and terms, correct
mistakes in good faith, oppose patent abuse, and firmly defend users and the
project when restraint is mistaken for weakness.
